...
def clean_data(raw_data):
    ...
    df = df[['name', 'date_utc', 'rocket', 'success', 'details']]
        ...
        with conn.cursor() as cur:
            for _, row in cleaned_df.iterrows():
                ...
                insert_query = sql.SQL(
                    """
                    INSERT INTO kev.spacex_launches (name, date_utc, rocket, success, details)
                    VALUES (%s, %s, %s, %s, %s)
                    ON CONFLICT (name) DO NOTHING;
                    """
                )
                cur.execute(insert_query, (
                    row['name'], row['date_utc'], row['rocket'], row['success'], row['details']
                ))
            conn.commit()
...
def etl_process():
    api_url = "https://api.spacexdata.com/v5/launches/latest"  # SpaceX latest launch API
